                        ...
                        onChange={(e)=>
                        {
                            setQuestions(questions.map((question, qIndex)=>
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                    if(ans2.id === ans.id)
                                        return {"id": ans.id.toString(), text: e.target.value, votes: ans.votes}
                                    else
                                        return ans2
                                    })
                                    return tmp;
                ...
                if(questions[parseInt(questionIndex)].answers.length<10)
                {
                    ...
                    setQuestions(questions.map((question, qIndex)=>{
                        ...
                        else
                        {
                            let tmp = question;
                            // tmp.answers[index]={id: tmp.answers.length.toString() ,text: e.target.value, votes: 0};
                            tmp.answers = ([...tmp.answers,{id:answers.length.toString(), text:'', voted: 0}])
                    ...
                    showAlert(true, 'danger', 'Too many answers!');
                }}>
                add new answer
            </button>
